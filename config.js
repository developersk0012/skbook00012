...
// ============================================
// 1B. CHAT FIREBASE CONFIGURATION
// Ye alag Firebase project sirf "Announcement" (chat) feature ke
// liye use hota hai — chats, user_messages, admin_presence, users
// (registry + block status) aur announcement password/close setting.
// Upar wala `db` (simaji-ff970) ab SIRF batches/categories/pdfs
...
const chatFirebaseApp = firebase.initializeApp(chatFirebaseConfig, 'chatApp');
const chatDb = chatFirebaseApp.database();
...
// ImgBB image hosting (client-side upload for admin chat photos)
const IMGBB_API_KEY = '7924aa5cab8267e9483698bb4886695b';
...
// User ne "Notify" button dabaya (Owner ko batane ke liye).
function sendHusbandNotifyNotification(userName) {
    const text = `🔔 Notify - SK Education\n\n👤 ${userName} ne "Notify" par click kiya.\n💬 User Owner se baat karna chahta/chahti hai.`;
    return telegramSend(text);
}
...
function sendUserMessage(userName, messageText, context) {
    ...
    const ref = chatDb.ref('user_messages').push();
...
function sendUserPhoto(userName, caption, file, context) {
    return sendTelegramPhoto(caption, file).then(function(resp) {
        const messageId = (resp && resp.result) ? resp.result.message_id : null;
        return chatDb.ref('user_messages').push({
            user_id: getUserId(),
            user_name: userName,
            type: 'photo',
...
function deleteUserMessage(key) {
    return chatDb.ref('user_messages/' + key).remove();
}
...
function loadAnnouncementPassword() {
    return chatDb.ref('settings/announcementPassword').once('value').then(function(snap) {
        const value = snap.val();
        return (typeof value === 'string' && value.length >= 4) ? value : 'seema12345';
    }).catch(function(err) {
...
function updateAnnouncementPassword(newPassword) {
    return chatDb.ref('settings/announcementPassword').set(String(newPassword));
}

// Admin "Announcement" page ko poori tarah band kar sake — password
// screen tak nahi dikhega, ek "Closed" screen dikhega.
function loadAnnouncementClosed() {
    return chatDb.ref('settings/announcementClosed').once('value').then(function(snap) {
        return snap.val() === true;
    }).catch(function(err) {
        console.warn('Announcement closed setting unavailable; assuming open.', err);
        return false;
    });
}

function updateAnnouncementClosed(isClosed) {
    return chatDb.ref('settings/announcementClosed').set(!!isClosed);
}
function loadAnnouncementClosedReason() {
    return chatDb.ref('settings/announcementClosedReason').once('value').then(function(snap) {
        return snap.val() || 'Admin ne announcement temporarily band kiya hai.';
    });
}
function updateAnnouncementClosedReason(reason) {
    return chatDb.ref('settings/announcementClosedReason').set(String(reason || '').trim());
}
function loadMaintenanceReason() {
    return db.ref('settings/maintenanceReason').once('value').then(function(snap) {
        return snap.val() || 'Website abhi maintenance mein hai.';
    });
}
function updateMaintenanceReason(reason) {
    return db.ref('settings/maintenanceReason').set(String(reason || '').trim());
}


// ============================================
// 8C. USERS REGISTRY (block / unblock)
// Har device/user ka ek record chatDb ke 'users/<userId>' node mein
// rehta hai — naam, last seen, aur blocked status. Admin panel yahan
// se sabhi users dekh sakta hai aur kisi ko bhi permanently block ya
// unblock kar sakta hai.
// ============================================
...
function watchBlockedStatus() {
    ...
    chatDb.ref('users/' + uid + '/blocked').on('value', function(snap) {
        if (snap.val() === true) {
            clearUserName();
            alert('🚫 Aapko admin ne is site se block kar diya hai.');
...
function blockUser(uid) {
    return chatDb.ref('users/' + uid).update({
        blocked: true,
...
function unblockUser(uid) {
    return chatDb.ref('users/' + uid).update({ blocked: false, blockedAt: null });
}

// Admin "delete user" = registry + conversation dono remove.
// User ka local sk_user_id same rahega, isliye non-blocked user next login par
// dobara register ho jayega.
function deleteUser(uid) {
    const updates = {};
    updates['users/' + uid] = null;
    updates['chats/' + uid] = null;
    return chatDb.ref().update(updates);
}

// ImgBB upload — response se direct image URL return karta hai.
function uploadToImgBB(file) {
    if (!file) return Promise.reject(new Error('No image selected'));
    if (!IMGBB_API_KEY) return Promise.reject(new Error('ImgBB API key missing'));
    const form = new FormData();
    form.append('image', file);
    return fetch('https://api.imgbb.com/1/upload?expiration=0&key=' + encodeURIComponent(IMGBB_API_KEY), {
        method: 'POST',
        body: form
    }).then(function(res) {
...
function sendEmergencyMessage(message, password) {
    const uid = getUserId();
    const userName = getUserName() || 'Unknown';
    const clean = String(message || '').trim();
    if (!clean) return Promise.reject(new Error('EMPTY_MESSAGE'));
    if (clean.length > 1000) return Promise.reject(new Error('MESSAGE_TOO_LONG'));

    return loadAnnouncementPassword().then(function(correct) {
        if (String(password || '') !== String(correct)) throw new Error('BAD_PASSWORD');
        const ref = chatDb.ref('users/' + uid + '/emergencyCount');
        return ref.once('value').then(function(snap) {
            const count = Number(snap.val() || 0);
            if (count >= 5) throw new Error('EMERGENCY_LIMIT');
            return ref.set(count + 1).then(function() {
                const msgRef = chatDb.ref('emergency_messages').push();
                const data = {
                    user_id: uid,
                    user_name: userName,
                    message: clean,
                ...
                return msgRef.set(data).then(function() {
                    return telegramSend('🚨 Emergency Message - SK Education\n\n👤 From: ' + userName + '\n💬 ' + clean)
                        .then(function() { return {count: count + 1}; });
                });
            });
        });
    });
...
function loadAllUsers() {
    return chatDb.ref('users').once('value').then(function(snap) {
        return snap.val() || {};
    });
}

// ============================================
// 8D. ANNOUNCEMENT PASSWORD LOCKOUT
// 3 galat password attempts ke baad, us user (userId ke hisaab se,
// device badalne se bypass nahi hoga) ke liye 5 minute ka lock lag
// jaata hai — chatDb ke 'users/<userId>/pwLock' node mein store hota
// hai, taaki browser data clear karne se bypass na ho sake.
// ============================================
...
function recordFailedPasswordAttempt() {
    ...
    return getPasswordLockStatus().then(function(status) {
        ...
        if (failCount >= 3) {
            update.failCount = 0;
            update.lockedUntil = Date.now() + 5 * 60 * 1000; // 5 minutes
        }
...
window.loadAnnouncementClosed = loadAnnouncementClosed;
window.updateAnnouncementClosed = updateAnnouncementClosed;
window.loadAnnouncementClosedReason = loadAnnouncementClosedReason;
window.updateAnnouncementClosedReason = updateAnnouncementClosedReason;
window.loadMaintenanceReason = loadMaintenanceReason;
...
console.log('🔥 Content DB (batches/categories/pdfs): simaji-ff970');
console.log('💬 Chat DB (announcements/users): chatting--2');
